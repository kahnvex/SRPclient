...
void setFrame(frame *f, int seqnum, int lframe, int dsize, int ack,
              char *data);
int readtoframe(char *c, const FILE **fp);
int SendNextFrames(int moveCount, frame frameArray[], int arraySize, int LB,
                   const FILE **fp, int *sock,
                   struct sockaddr *remoteServAddr, timeStruct timesArray[],
                   int timeSize);
...
int PlaceFrame(frame f, frame frameArray[], int arraySize, int *LB, int *RB,
               FILE *fp);
int ServerMoveForward(frame frameArray[], int arraySize, int *LB, int *RB,
                      FILE *fp);
...
void storeFrame(frame frameBuffer[], frame *f, int LB, int RB, int size)
{
    if (LB < RB)
    {
        ...
        if ((*f).seqNum >= LB && (*f).seqNum < RB)
        {
            ...
            {
                setFrame(&frameBuffer[(*f).seqNum], (*f).seqNum,
                         (*f).lastFrame, (*f).dataSize, 1, (*f).data);
            }
        }
    }
                    ...
                    int timeSize )
{
    ...
    if (startSeq < finishSeq)
    {
        ...
        for (i = startSeq; i < finishSeq; i++)
        {
            ...
            /* Send it */
            if ((sendto_(*sock, sendBuffer, strlen(sendBuffer), 0,
              remoteServAddr, sizeof(*remoteServAddr))) < 0 )
            {
